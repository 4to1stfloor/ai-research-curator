...
분량 규칙 (매우 중요):
- 문단1(한 줄 개요): 1문장
- 문단2(연구 배경): 최소 3~4문장, 350자 이상. 질환/분야 맥락과 기존 한계를 구체적으로.
- 문단3(핵심 발견): 최소 4~5문장, 500자 이상. 사용된 model/cohort/method, 주요 결과, 수치·marker·pathway 이름을 구체적으로.
- 문단4(의의): 최소 3~4문장, 350자 이상. 학문적·임상적 함의와 향후 전망.
- 전체 목표: 1500~2200자. 짧게 요약하려 하지 말고 정보가 있는 만큼 충실하게 서술하세요.
- 단, 원문에 없는 내용을 지어내서 늘리지는 마세요."""
...
**분량 규칙:**
초록만 있어도 짧게 넘기지 말고, 초록에 있는 정보를 최대한 풀어서 서술하세요.
- 문단2(배경): 최소 3~4문장, 350자 이상. 질환/분야의 맥락과 이 연구가 필요한 이유.
- 문단3(핵심 발견): 최소 4~5문장, 450자 이상. 초록에 언급된 method/model/marker/결과를 하나씩 구체적으로.
- 문단4(의의): 최소 3~4문장, 300자 이상. 학문적·임상적 함의.
- 목표 전체 길이: 1200~1800자. 초록에 없는 내용을 추측해서 채우면 안 되지만, 초록에 있는 정보는 최대한 풀어쓰세요.
...
```
#### Figure N: <Figure 제목>
**원문 해석**: <Figure Legend 원문을 한국어로 풀어쓴 문단, 모든 Panel 포함>
**핵심 내용**: <이 Figure가 보여주는 주요 메시지 1문장>
**세부 설명**:
- Panel A: <Panel A 설명>
- Panel B: <Panel B 설명>
- Panel C: <Panel C 설명>
... (Legend에 있는 모든 Panel을 마지막까지 빠뜨리지 말고 나열. Panel K까지 있으면 K까지, 소문자면 소문자로.)
```
...
- **모든 Panel을 빠짐없이 포함하세요**. Legend 원문에 있는 Panel 표시 (a, b, c, ..., k, l, m, ... 또는 A, B, C, ...) 를 하나도 빠뜨리지 말고 각각 한국어로 서술하세요. 예: legend에 Panel a-k가 있으면 세부 설명에도 반드시 a부터 k까지 11개 항목이 있어야 합니다.
...
- 결과 길이 가이드: 원문 legend가 길다면 (수백자 이상) 원문 해석도 그에 맞게 여러 문장으로 작성.
- **절대 금지**: "(원문 legend 없음)" 같은 placeholder를 출력하지 마세요. 이 함수는 legend가 있을 때만 호출됩니다. 만약 특정 Figure의 legend가 위에 없다면 그 Figure는 아예 출력하지 마세요.
